package com.czajkak.ServiceBookingSystem.controller;

import com.czajkak.ServiceBookingSystem.dto.AuthenticationRequest;
...
import com.czajkak.ServiceBookingSystem.services.authentication.AuthService;
import com.czajkak.ServiceBookingSystem.services.jwt.UserDetailsServiceImpl;
import com.czajkak.ServiceBookingSystem.utill.JwtUtil;
...
//    }
    ...
    @PostMapping("/client/sign-up")
    public ResponseEntity<?> signUpClient(@Valid @RequestBody SignupRequestDTO signupRequestDTO, BindingResult result) {
        if (result.hasErrors()) {
...
//    }
    ...
    @PostMapping("/company/sign-up")
    public ResponseEntity<?> signupCompany(@Valid @RequestBody SignupRequestDTO signupRequestDTO, BindingResult result) {
        if (result.hasErrors()) {
            StringBuilder errorMessage = new StringBuilder();
            for (FieldError error : result.getFieldErrors()) {
                errorMessage.append(error.getField() + ": " + error.getDefaultMessage() + "\n");
